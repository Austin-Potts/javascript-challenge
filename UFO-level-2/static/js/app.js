...
//---------------------------------------------------------------------------------------------------------------------------------
//function to change text and placeholder for different filter selections
filterOption.on("change", function() {

    //grabbing value from filter_option id in html
    var filterValue = filterOption.property("value");
    d3.select("#changefilter").node().value = '';

    // Setting placeholder values for input text using switch case
    switch (filterValue) {
        case 'datetime':
    ...
    //formatting for the placeholder in the search box and text above
    d3.select("input").attr("placeholder", placeHolder);
    d3.select("label")
      .attr("for",filterValue)
      .text(`Enter a ${filterValue} that you wish to investigate.`);
});
